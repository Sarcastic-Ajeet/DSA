<h1> <a href = "https://leetcode.com/problems/course-schedule-iv/description/">1462. Course Schedule IV</a></h1>
<div><div class="elfjS" data-track-load="description_content"><p>There are a total of <code>numCourses</code> courses you have to take, labeled from <code>0</code> to <code>numCourses - 1</code>. You are given an array <code>prerequisites</code> where <code>prerequisites[i] = [a<sub>i</sub>, b<sub>i</sub>]</code> indicates that you <strong>must</strong> take course <code>a<sub>i</sub></code> first if you want to take course <code>b<sub>i</sub></code>.</p>
...
public:
    vector<bool> checkIfPrerequisite(int n, vector<vector<int>>&edges, vector<vector<int>>& queries) {
       ...
       // edge[0] -> edge[1] = true; 
        ...
        // edge[i] -> edge[mid] = true;
        // edge[mid] -> edge[k] = true;
        // than only edge[i][j] = true;
        for(int mid = 0 ;mid < n ; mid++){
            for(int i = 0; i < n ; i++){
                for(int j = 0; j < n ; j++){
                    dist[i][j] = dist[i][j] || (dist[i][mid] && dist[mid][j]);
                }
            }
        }
        ...
        for(auto query : queries){
            result.push_back(dist[query[0]][query[1]]);
        }
        return result;
